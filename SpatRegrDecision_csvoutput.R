SpatialRegressionDecision<-function(Formula, DataFrame, Neighbor, OutputName, outTable){
	...
	if(!is.null(SARResult$type))
	{
		...
		outTable<- rbind(outTable, c(Formula, SARResult$type, paste0(round(SARResult$coefficients[2],4), Helper_AssignSignCode(EstimatepValue[2])), round(summary(OLS)$adj.r.squared,4), round(PseudoRSquared, 4), paste0(" ",round(SARResult$LL,4),"/", round(SARResult$logLik_lm.model,4)), paste0(round(AIC(SARResult),4),"/",round(SARResult$AIC_lm.model),4), round(SpatialAutoregressiveCoefficient,4), paste0(round(Wald1.sarlm(SARResult)$statistic,4),Helper_AssignSignCode(Wald1.sarlm(SARResult)$p.value)),paste0(round(LR1.sarlm(SARResult)$statistic,4),Helper_AssignSignCode(LR1.sarlm(SARResult)$p.value))))
		#cat(2 * (1 - pnorm(abs(SARResult$coefficients/SARResult$rest.se))))
	}
	else
	{
		#If OLS is used, report partial results
		outTable<- rbind(outTable, c(Formula, "OLSResult", paste0(OLS$coefficients[2],Helper_AssignSignCode(summary(OLS)$coefficients[2,4])), summary(OLS)$adj.r.squared, "PseudoRSquared", "LogLik_PH", "AIC_PH", "SAC_PH", "WaldT_PH", "LRTest_PH"))
	}
	return(outTable)
}
